...
return [
    ...
    */
    ...
    |--------------------------------------------------------------------------
    |
    | 需要「爬虫 → PHP 直出，真人 → SPA」分流的 H5 路径前缀列表。
    | 框架默认空（不渲染任何直出 location）；项目层（如 scrm）在
    | config/domain.php 覆盖落地，例如：
    |   ['h5/pages/course/detail', 'h5/pages/course/index',
    |    'h5/pages/shop/detail', 'h5/pages/event/detail']
    | 分流判定用 $is_seo_or_ai_bot（仅内容优化，不承载安全边界）。
    */
    'seo_direct_out_paths' => [],
